...
const stripe = require("stripe")(
  "sk_test_51Mv1NQBkeolJlHlyoRWR84DNgMJlnErnWVV815de3LZDsCOi5dAEtv9BV09jxOAnYhNWY44IoQXv891HxFSQgiD400Qin9N9gX"
);
...
const getSingleOrder = async (req, res) => {
  ...
  if (!orderId) throw new BadRequest("Please provide an order id");
  ...
  if (!order) throw new NotFound(`Order with id ${orderId} doesn't exists`);
...
const createOrder = async (req, res) => {
  ...
  if (!tax || !shippingFee)
    ...
    if(!Number.isInteger(tax) || !Number.isInteger(shippingFee))
        throw new BadRequest("Tax and shipping fee must be number type!");
